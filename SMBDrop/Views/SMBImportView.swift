...
struct SMBImportView: View {
    ...
    private var destinationsView: some View {
        List {
            Section {
                ...
                Text("Choose a share, browse its folders, then select files to save on this iPhone.")
            }
        }
    }
    ...
    private func detail(for item: SMBRemoteItem) -> String {
        ...
        if let modificationDate = item.modificationDate {
            details.append(modificationDate.formatted(date: .abbreviated, time: .omitted))
        }
